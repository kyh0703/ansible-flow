import { Controller, Post, Body, Res, Req, UseGuards, Get, HttpCode } from '@nestjs/common';
...
export class AuthController {
  ...
  async refresh(@Req() req: Request, @Res({ passthrough: true }) res: Response) {
    const user = req.user;
    const { accessToken, refreshToken } = await this.authService.generateTokens(user.userId, user.email);
    this.setCookieWithRefreshToken(res, refreshToken);
    return { accessToken };
  }
  ...
  async googleAuthCallback(@Req() req, @Res({ passthrough: true }) res: Response) {
    ...
    this.setCookieWithRefreshToken(res, refreshToken);
    res.redirect(`${process.env.FRONTEND_URL || 'http://localhost:5173'}?token=${accessToken}`);
  }
  ...
  @Get('kakao/callback')
  @UseGuards(KakaoAuthGuard)
  async kakaoAuthCallback(@Req() req, @Res({ passthrough: true }) res: Response) {
    const { accessToken, refreshToken } = await this.authService.oauthLogin(req);
    this.setCookieWithRefreshToken(res, refreshToken);
    res.redirect(`${process.env.FRONTEND_URL || 'http://localhost:5173'}?token=${accessToken}`);
  }
