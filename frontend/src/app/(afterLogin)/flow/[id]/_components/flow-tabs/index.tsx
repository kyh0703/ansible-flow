...
import { getFlowPath } from '@/utils/route-path'
import {
  DragDropContext,
  Draggable,
  DropResult,
...
export default function FlowTabs() {
  ...
  const handleClose = useCallback(
    (event: React.MouseEvent, closeFlowId: number) => {
      ...
      removeHistoryMutate({ flowId: closeFlowId })
  ...
  const handleCloseOthers = useCallback(
    (event: React.MouseEvent, closeSubFlowId: number) => {
      ...
      for (const subFlowId of result.closedSubFlowIds) {
        removeHistoryMutate({ flowId })
      }
      router.push(getFlowPath(result.subFlowId))
    },
  ...
  const handleTabClick = (index: number) => {
    ...
    if (subFlow) {
      router.push(getFlowPath(subFlow.id))
    }
  }
